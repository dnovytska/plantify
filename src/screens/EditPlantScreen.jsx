...
export default function EditPlantScreen() {
  ...
  const loadPlantTypesFromLocalDb = async (database) => {
    try {
  ...
  const handleCreatePlantType = async () => {
    ...
    try {
      const watering = await db.getFirstAsync('SELECT idwatering_level FROM watering_levels WHERE name = ?', [newPlantWatering || 'Moderada']);
      const sunlight = await db.getFirstAsync('SELECT idsunlight_level FROM sunlight_levels WHERE name = ?', [newPlantSunlight || 'Meia Sombra']);
      const growthRate = await db.getFirstAsync('SELECT idgrowth_rate FROM growth_rates WHERE name = ?', [newPlantGrowthRate || 'Médio']);
      const careLevel = await db.getFirstAsync('SELECT idcare_level FROM care_levels WHERE name = ?', [newPlantCareLevel || 'Moderado']);
      ...
      const newPlantDetails = {
        ...
        type: 'existing',
        watering: newPlantWatering || 'Moderada',
        sunlight: newPlantSunlight || 'Meia Sombra',
        growth_rate: newPlantGrowthRate || 'Médio',
        care_level: newPlantCareLevel || 'Moderado',
      };
